...
const OrderForm = () => {
  ...
  const UsdtApprove = async () => {
    try {
      let tokens = ((tokensPrice * quantity * 10 ** 14) / TokenToUSD)
        .toFixed(0)
        .toString();
      setcheckTokens(tokens);
      await UsdtContract?.methods
        .approve(contractAddr, tokens)
  ...
  const BuyToken = async () => {
    try {
      await loadchain?.methods
        .Buyers(tokenType, quantity, price)
        .send({ from: accounts[0] });
    } catch (error) {
      ...
      <form>
        <div className="sell-blance">
          ...
          <div className="form-label blance">
            ...
            <p className="text-warning">{totaltoken}</p>
          </div>
